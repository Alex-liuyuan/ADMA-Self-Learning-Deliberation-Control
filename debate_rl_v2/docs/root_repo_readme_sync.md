...
- `mdt_game/`：基于 `debate_rl_v2/framework` 构建的独立 MDT 多学科肿瘤讨论场景包。
...
├─────────────────────────────────────────────────────────────────────────────┤
│  A. Legacy line                                                            │
│     debate_rl/ + scripts/*.py                                              │
│     早期训练、评估、可视化脚本更完整                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│  B. Main framework line                                                    │
│     debate_rl_v2/                                                          │
│     RL strategy control + LLM generation + compliance + online adaptation  │
├─────────────────────────────────────────────────────────────────────────────┤
│  C. Scenario package line                                                  │
│     mdt_game/                                                              │
│     基于 debate_rl_v2/framework 的独立 MDT 场景实现                        │
├─────────────────────────────────────────────────────────────────────────────┤
│  Shared assets                                                             │
│     configs/ checkpoints/ runs/ figures/ tests/                            │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
state
  ...
  -> compliance verification
  -> reward
  -> training update / online override
  -> distillation / skill DB / prompt evolution / causal graph
```

### 3.4 子图 C：`mdt_game` 在仓库中的位置

```text
debate_rl_v2/framework
  ├─ GameScenario
  ├─ GameEngine
  ├─ RoleRegistry
  ├─ GameToolRegistry
  └─ BaseStrategyBridge / BaseObservationEncoder
                │
                ▼
             mdt_game
  ├─ MDT_ROLES
...
├── mdt_game/                # 基于 framework 的独立 MDT 场景包
├── requirements.txt         # 依赖
...
2. `mdt_game/README.md`
3. `debate_rl_v2/envs/fusion_env.py`
4. `debate_rl_v2/core/strategy_bridge.py`
5. `debate_rl_v2/core/compliance_verifier.py`
6. `debate_rl_v2/agents/llm_agent.py`
7. `debate_rl_v2/mode/controller.py`
8. `debate_rl_v2/mode/online_updater.py`
9. `mdt_game/scenario.py`
...
from debate_rl_v2.core.strategy_bridge import (
    StrategyBridge,
    TOTAL_OBS_DIM,
    PROPOSER_CTRL_ACT_DIM,
    CHALLENGER_CTRL_ACT_DIM,
    ARBITER_CTRL_ACT_DIM,
    COORDINATOR_ACT_DIM,
)
...
rl_agents = MADDPGAgentGroup(
    obs_dims={
        "proposer_ctrl": TOTAL_OBS_DIM,
        "challenger_ctrl": TOTAL_OBS_DIM,
        "arbiter_ctrl": TOTAL_OBS_DIM,
        "coordinator": TOTAL_OBS_DIM,
    },
    act_dims={
        "proposer_ctrl": PROPOSER_CTRL_ACT_DIM,
        "challenger_ctrl": CHALLENGER_CTRL_ACT_DIM,
        "arbiter_ctrl": ARBITER_CTRL_ACT_DIM,
        "coordinator": COORDINATOR_ACT_DIM,
    },
)
...
如果你要直接运行 `framework` 主线下的 MDT 场景，优先使用 `mdt_game/`：
...
- 直接基于 `GameScenario + GameEngine` 运行。
- 角色定义、患者生成、医学工具和奖励都在 `mdt_game/` 中独立实现。
- 更适合做新场景扩展、论文实验和 framework 设计验证。
...
**它是一套把强化学习用于“控制多智能体语言协作策略”，再把大语言模型用于“执行自然语言推理与表达”，并支持通过具体场景包落地为可运行任务系统的研究型融合框架。**
