...
├─────────────────────────────────────────────────────────────────────────────┤
│  Scene Definition                                                           │
│    __init__.py              MDT_ROLES + create_mdt_scenario()              │
│    scenario.py              MDTScenario                                    │
├─────────────────────────────────────────────────────────────────────────────┤
│  Episode Source                                                             │
│    patient_generator.py     SyntheticPatientGenerator + PatientCase         │
├─────────────────────────────────────────────────────────────────────────────┤
│  Runtime Engine                                                             │
│    GameEngine                 通用博弈执行                                  │
│    DashboardGameRunner        带实时仪表盘的执行器                          │
├─────────────────────────────────────────────────────────────────────────────┤
│  Agent Side                                                                 │
...
├─────────────────────────────────────────────────────────────────────────────┤
│  Optional RL Layer                                                          │
│    observation.py          MDTObservationEncoder                           │
│    strategy.py             MDTStrategyBridge + MDTStyleComposer            │
├─────────────────────────────────────────────────────────────────────────────┤
│  Evaluation                                                                  │
│    scenario.update_state() + scenario.compute_rewards()                    │
│    奖励中可调用指南/风险/药物交互工具                                      │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 子图 A：一次 episode 的完整流

```text
SyntheticPatientGenerator.generate()
        │
        ▼
PatientCase
        │ to_summary() / to_dict()
...
MDTScenario.create_episode()
        │
        ▼
initial CollaborationState.metadata
        │
        ▼
GameEngine.run_episode()
  ├─ build_role_prompt(oncologist)
  ├─ build_role_prompt(surgeon)
  ├─ build_role_prompt(radiologist)
  ├─ build_role_prompt(pathologist)
  ├─ LLMAgent.act(...)
  ├─ scenario.update_state(...)
  ├─ scenario.check_terminal(...)
...
│ propose        │         │ challenge    │         │ challenge       │
│ 治疗方案        │         │ 手术风险评估 │         │ 放疗/影像评估    │
└──────┬────────┘         └──────┬───────┘         └────────┬───────┘
       │                         │                            │
       │ tool calls              │ tool calls                 │ optional tools
       ▼                         ▼                            ▼
query_nccn_guidelines   calculate_risk_score        search_clinical_trials
       │                         │                            │
       └──────────────┬──────────┴───────────────┬────────────┘
                      ▼                          ▼
                ┌────────────────────────────────────┐
                │          pathologist                │
                │ evaluate / verdict / quality_score │
                └────────────────────────────────────┘
```
...
MDTObservationEncoder
  ...
  └─ role_obs:   16D + 4D = 20D
   ...
   ▼
RL agents
  ├─ oncologist   4D action
  ├─ surgeon      4D action
  ├─ radiologist  3D action
  └─ pathologist  3D action
   │
   ▼
MDTStrategyBridge.translate()
  ├─ temperature
  └─ role-specific style dimensions
       ├─ oncologist: aggressiveness / evidence / guideline
       ├─ surgeon: conservatism / detail / risk_tolerance
       ├─ radiologist: thoroughness / specificity
       └─ pathologist: strictness / evidence_weight
   │
   ▼
MDTStyleComposer.compose()
   │
   ▼
LLMAgent._style_directive / client.temperature
```
...
```text
路径 A：framework 主线
  mdt_game/scripts/run_mdt_game.py
    -> create_mdt_scenario()
    -> GameEngine / DashboardGameRunner
    -> LLMAgent(role_definition=MDT_ROLES[role])

路径 B：兼容示例线
  mdt_game/examples/run_mdt_debate.py
  mdt_game/examples/run_mdt_with_dashboard.py
    -> debate_rl_v2.envs.TextDebateEnv
    -> 固定 MDT 肺癌案例
```
